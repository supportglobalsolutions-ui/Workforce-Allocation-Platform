...
/** Confirm, then enter the emailed OTP, then the RDP machine is permanently removed. */
export default function DeleteRdpModal({
  machine,
  onClose,
  onDeleted,
}: {
  ...
  async function sendCode() {
    ...
    setError(null);
    try {
      const res = await api.post<Challenge>(`/rdp/${machine.id}/delete/request-otp`, {});
      setChallenge(res);
    } catch (e) {
  ...
  async function confirmDelete(submitted = code) {
    ...
    setError(null);
    try {
      await api.post(`/rdp/${machine.id}/delete/confirm`, {
        challenge_id: challenge.challenge_id,
        code: submitted.trim(),
      });
      onDeleted();
    } catch (e) {
      setError(e instanceof Error ? e.message : 'Could not delete this RDP machine.');
    } finally {
      setBusy(false);
    }
  }
  ...
  return createPortal(
    <div className="fixed inset-0 z-[110] flex items-center justify-center p-4" role="dialog" aria-modal="true">
      <button
        ...
        disabled={busy}
      />
      <div className="glass-modal relative z-10 w-full max-w-lg overflow-hidden rounded-2xl border border-theme shadow-2xl">
          ...
          {step === 'confirm' ? (
            <>
              <p className="text-sm text-theme-heading">
                Permanently remove this machine from the platform.
              </p>
              <ul className="text-xs text-theme-muted space-y-1 list-disc pl-4">
                <li>Open sessions are ended and the seat is freed.</li>
                <li>The Guacamole connection is removed.</li>
                <li>Session history stays; it is unlinked from this machine.</li>
              </ul>
              <p className="flex items-start gap-1.5 text-xs text-theme-muted">
                <Mail size={12} className="shrink-0 mt-0.5" />
                A verification code will be sent to the Settings alert email.
              </p>
            ...
            <>
              {challenge ? (
                <p className="text-sm text-theme-heading">
                  ...
                  <span className="font-semibold">{challenge.sent_to}</span>.
              ...
              <p className="text-[11px] text-theme-muted">
                {!challenge
                  ? 'The code field will activate as soon as delivery is confirmed.'
                  : left > 0
                    ? `Expires in ${left}s`
              ...
              <div className="flex justify-end gap-2 pt-1">
                <button
                  type="button"
                  onClick={() => void sendCode()}
                  disabled={busy || !challenge || left > 150}
                ...
                <button
                  ...
                  disabled={busy || !challenge || code.length < 6 || left === 0}
                  className="text-sm py-2 px-4 inline-flex items-center gap-2 rounded-xl font-semibold bg-danger/90 text-white hover:bg-danger disabled:opacity-50"
                >
