...
interface RdpSessionViewProps {
  rdpId: string;
  /**
   * Portal this view is mounted under, so "back to claim board" returns to the
   * caller's own shell instead of dropping an executive into the worker one.
   */
  basePath?: RdpPortal;
}

export default function RdpSessionView({ rdpId, basePath = '/worker' }: RdpSessionViewProps) {
      ...
      ch.onmessage = (e) => {
        if (e.data?.type === 'session-ended' && e.data?.rdpId === rdpId) {
          const dest =
            ...
            sessionEvidenceUrl(rdpId, null, basePath);
  ...
  const handleEndConnection = useCallback(async () => {
    ...
    const dest = sessionEvidenceUrl(rdpId, sid, basePath);
  ...
  if (error && !machine) {
    return (
      <div className="max-w-6xl">
        ...
        <Link href={`${basePath}/rdp-claim-board`} className="btn-secondary text-sm mt-4 inline-block">
          Back to claim board
        </Link>
      </div>
    );
  ...
  return (
    <div className="max-w-6xl space-y-4">
      ...
      <div className="glass-panel p-5 space-y-3">
        ...
        <button
          type="button"
          onClick={() => openRdpDesktopTab(rdpId, basePath)}
      ...
      <div>
        <Link href={`${basePath}/rdp-claim-board`} className="btn-secondary text-sm">
          Claim board
        </Link>
      </div>
    </div>
